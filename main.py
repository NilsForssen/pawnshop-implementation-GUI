...
from pawnshop import ChessBoard, Pieces, GameNotations, ChessVector, Exceptions
from Network import Network
from socket import gaierror
from Utils import getResourcePath, fetchImage, squeek
import SoundQueue


class LoadingDialog(tk.Toplevel):
    def __init__(self, master, txt, GIFPath=getResourcePath("sprites\\")):
        ...
        gif = Image.open(GIFPath + "LoadingGIF.gif")
        self._frames = []
...
class ChessGame(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        # Chesscanvas related
        self.boardInits = {
            "Empty": ChessBoard.Board,
            "Classic": ChessBoard.initClassic,
            "4P": ChessBoard.init4P
        }
    ...
    def drawBoard(self):
        ...
        for i, piece in enumerate(self.board):
            color = ("saddle brown", "white")[(i + int((i) / self.board.getCols())) % 2 == 0]
            if not isinstance(piece, Pieces.Disabled):
                pos = piece.vector
    ...
    def hostGame(self):
        ...
        self.loadDialog = LoadingDialog(self, txt=f"Waiting for other player to connect to your IP.")
    ...
    def _waitForGame(self):
        ...
        if isinstance(echo, ChessBoard.Board):
            try:
                self.loadDialog.close()
                del self.loadDialog
    ...
    def _updateBoard(self):
        ...
        if newBoard is None:
            ...
            if newBoard != self.board:
                self.board = newBoard
                squeek(self.board.getHistory()[-1])
    ...
    def chessInteract(self, event, *args):
        if not self.mated and self.board.ready:
            if event.x + self.blackbarRight < self.boardCanv.winfo_width() and event.x - self.blackbarLeft > 0 and event.y + self.blackbarBottom < self.boardCanv.winfo_height() and event.y - self.blackbarTop > 0:
                vec = ChessVector.ChessVector((int((event.y - self.blackbarTop) / self.square), int((event.x - self.blackbarRight) / self.square)))
                ...
                try:
                    ...
                    elif isinstance(piece, Pieces.Piece) and piece.color == self.board.currentTurn and (self.clientColor is None or self.clientColor == piece.color):
                        self.select(piece, highlights=piece.getMoves(self.board))
                    else:
                        self.clearHighlights()

                    if any(self.board.getCheckmates().values()):
                        while True:
                            if len(self.board.getCheckmates().keys()) > 2:
                                for color, value in self.board.getCheckmates().items():
                                    if value:
                                ...
                                messagebox.showinfo("Checkmate!", f"{[color for color, value in self.board.getCheckmates().items() if value].pop()} has been checkmated!")
                                self.mated = True
                                break
        ...
        try:
            ...
            while True:
                ...
                if prompt is None:
                    break
                try:
                    pType = {pType.__name__: pType for pType in self.board.getPromoteTo(self.selected.color)}[prompt.lower().capitalize()]
                except KeyError:
                    msg = prompt + "is Not a valid piece, must be any of \n" + "\n".join([pType.__name__ for pType in self.board.getPromoteTo(self.selected.color)])
                    continue
                else:
    ...
    def highlightKings(self):
        if any(self.board.getChecks().values()):
            for color in [col for col, value in self.board.getChecks().items() if value]:
                for king in self.board.getKings()[color]:
    ...
    def resize(self, *args):
        self.update_idletasks()
        rankH = int(self.boardCanv.winfo_width() / self.board.getRows())
        fileH = int(self.boardCanv.winfo_height() / self.board.getCols())
        self.square = (rankH, fileH)[rankH > fileH]
        self.blackbarLeft = int((self.boardCanv.winfo_width() - (self.board.getRows() * self.square)) / 2)
        self.blackbarRight = self.boardCanv.winfo_width() - self.blackbarLeft - (self.board.getRows() * self.square)
        self.blackbarTop = int((self.boardCanv.winfo_height() - (self.board.getCols() * self.square)) / 2)
        self.blackbarBottom = self.boardCanv.winfo_height() - self.blackbarTop - (self.board.getCols() * self.square)

        self.updateGraphics()

    def updateGraphics(self):
        """Update all widgets of frame"""
        self.historyString.set(GameNotations.readable(self.board.getHistory(), len(self.board.getTurnorder())))  # this should be a getHistory function
        self.photos = []
